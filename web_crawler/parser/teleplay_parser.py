...
from comm.comm import teleplay_field, date_str, teleplay_url


# 电视剧解析
from model.video import Video

class TeleplayParser(object):
    ...
    def __parse_drama_num(self, video, soup):
        drama_list_tag = soup.find('ul', 'linkpanel')
        catahandle_tag = drama_list_tag.find(class_='catahandle')
        if catahandle_tag is not None:
            catahandle_tag.extract()
    ...
    def __get_all_suffixes(self):
        url_helper = TeleplayUrlHelper(self.__video_url)
        # 当尝试超过失败次数时，则跳过
...
class TeleplayUrlHelper(object):
    ...
    def get_all_suffixes(self):
        ...
        suffixes_tmp = [( period_codes[0], category_codes[0], area_codes[0])]
        all_suffixes = []
        while len(suffixes_tmp) != 0:
            suffix_tuple = suffixes_tmp.pop()
            suffix = '_' + suffix_tuple[0] + '_' + suffix_tuple[1] + '_' + suffix_tuple[2] + '_1_'
            video_num = TeleplayUrlHelper.get_num_of_video(self.__page_prefix + suffix + '1.html')
            ...
            if suffix_tuple[2] == area_codes[0]:
                for area_code in area_codes[1:]:
                    suffixes_tmp.append((suffix_tuple[0], suffix_tuple[1], area_code))
            elif suffix_tuple[0] == period_codes[0]:
                for period_code in period_codes[1:]:
                    suffixes_tmp.append((period_code, suffix_tuple[1], suffix_tuple[2]))
